...
public class UUIDGenerator implements IdentifierGenerator {
	 ...
	 * 
	 * @see
	 * org.uorm.dao.id.IdentifierGenerator#generate(org.uorm.dao.dialect.Dialect
	 * , org.uorm.dao.common.ICommonDao, java.lang.Object,
	 * org.uorm.orm.annotation.FieldMapping, boolean)
	 */
	@Override
	public Serializable generate(Dialect dialect, ICommonDao dao, Object pojo,
		...
		try {
			Map<String, PropertyDescriptor> props = ObjectMappingCache
					.getInstance().getObjectPropertyMap(pojo.getClass());
			...
			if (UUID.class.isAssignableFrom(fieldType)) {
				...
				System.arraycopy(
						Utils.long2bytes(uuid.getMostSignificantBits()), 0,
						bytes, 0, 8);
				System.arraycopy(
						Utils.long2bytes(uuid.getLeastSignificantBits()), 0,
						bytes, 8, 8);
				idval = bytes;
			} else {
				throw new SQLException("Unanticipated return type ["
						+ fieldType.getName() + "] for UUID conversion");
			}
			if (writevalue) {
	 ...
	 * @see
	 * org.uorm.dao.id.IdentifierGenerator#generate(org.uorm.dao.dialect.Dialect
	 * , org.uorm.dao.common.ICommonDao, java.lang.Class, java.util.Map,
	 * org.uorm.orm.annotation.FieldMapping, boolean)
	 */
	@Override
	public Serializable generate(Dialect dialect, ICommonDao dao, Class<?> cls,
		...
		try {
			Map<String, PropertyDescriptor> props = ObjectMappingCache
					.getInstance().getObjectPropertyMap(cls);
			PropertyDescriptor prop = props.get(idFieldInfo.columnName());
			Class<?> fieldType = prop.getPropertyType();
			UUID uuid = UUIDHelper.generateUUID(simplerandom);
			Serializable idval = null;
	 ...
	 * @see
	 * org.uorm.dao.id.IdentifierGenerator#generate(org.uorm.dao.dialect.Dialect
	 * , org.uorm.dao.common.ICommonDao, java.lang.Class,
	...
	public Serializable[] generate(Dialect dialect, ICommonDao dao,
		...
		for (int i = 0; i < vals.length; i++) {
			vals[i] = generate(dialect, dao, cls, valmaps[i], idFieldInfo,
					writevalue);
		}
		return vals;
	}
