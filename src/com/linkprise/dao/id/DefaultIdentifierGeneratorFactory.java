...
import com.linkprise.utils.PropertyHolderUtil;
...
public class DefaultIdentifierGeneratorFactory implements
	...
	public DefaultIdentifierGeneratorFactory() {
		...
		generator2ClassMap.put(KeyGenertator.IDENTITY,
				com.linkprise.dao.id.IdentityGenerator.class);
		generator2ClassMap.put(KeyGenertator.UUID,
				com.linkprise.dao.id.UUIDGenerator.class);
		generator2ClassMap.put(KeyGenertator.UUIDHEX,
				com.linkprise.dao.id.UUIDHexGenerator.class);
		generator2ClassMap.put(KeyGenertator.GUID,
				com.linkprise.dao.id.GUIDGenerator.class);
		generator2ClassMap.put(KeyGenertator.INCREMENT,
				com.linkprise.dao.id.IncrementGenerator.class);
		generator2ClassMap.put(KeyGenertator.SEQUENCE,
				com.linkprise.dao.id.SequenceGenerator.class);
		generator2ClassMap.put(KeyGenertator.SELECT,
				com.linkprise.dao.id.SelectGenerator.class);
		generator2ClassMap.put(KeyGenertator.NATIVE,
				com.linkprise.dao.id.NativeGenerator.class);
	 ...
	 * 
	 * @see
	 * org.uorm.dao.id.IdentifierGeneratorFactory#createIdentifierGenerator(
	 * java.lang.String)
	 */
	@Override
	public IdentifierGenerator createIdentifierGenerator(String keygenertator)
			throws Exception {
		IdentifierGenerator generator = generator2IdentifierGeneratorInstMap
				.get(keygenertator);
		if (generator != null) {
			return generator;
		}
		...
		if (cls != null) {
			...
			if (idgen instanceof UUIDGenerator) {
				...
				String prop = PropertyHolderUtil
						.getProperty("ID.UUIDGenerator.random");
				...
				// set cust property
				String prop = PropertyHolderUtil
						.getProperty("ID.SelectGenerator.tablename");
				if (prop != null && prop.trim().length() > 0) {
					((SelectGenerator) idgen).setTableName(prop);
				}
				prop = PropertyHolderUtil
						.getProperty("ID.SelectGenerator.namecolumn");
				if (prop != null && prop.trim().length() > 0) {
					((SelectGenerator) idgen).setNameColumn(prop);
				}
				prop = PropertyHolderUtil
						.getProperty("ID.SelectGenerator.valuecolumn");
				if (prop != null && prop.trim().length() > 0) {
			...
			try {
				...
				if (ins instanceof IdentifierGenerator) {
					...
					generator2IdentifierGeneratorInstMap.put(keygenertator,
							(IdentifierGenerator) ins);
	 ...
	 * 
	 * @see
	 * org.uorm.dao.id.IdentifierGeneratorFactory#getIdentifierGeneratorClass
	 * (java.lang.String)
	 */
	@Override
